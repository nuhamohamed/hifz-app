...
function processCommitWords(rawText) {
  ...
  if (expectedWords.length === 0) {
    ...
    const done = [pendingCarry, accumulatedText, rawText].filter(Boolean).join(' ');
    pendingCarry = '';
    accumulatedText = '';
    matchPos = 0;
    return done;
  ...
  for (let ti = 0; ti < normWords.length && matchPos < expectedWords.length; ti++) {
    ...
    if (normWords[ti] === ew) {
      ...
      matchPos + 1 < expectedWords.length &&
      normWords[ti] === expectedWords[matchPos + 1]
    ) {
      // One-word STT omission — accept the next expected word and continue.
      matchPos += 2;
      if (matchPos >= expectedWords.length) {
        completionAt = ti;
        break;
    ...
    // else: insertion — skip silently, don't advance matchPos
  }
...
function handleServerMessage(event) {
  ...
  switch (msg.message) {
    ...
    case 'AddTranscript': {
      ...
      // Reveal committed words (stable, no prediction).
      callbacks.onCommit?.(accumulatedText);
...
export async function startListening({
  ...
  callbacks = { getExpectedWords, onAyahComplete, onPartial, onCommit, onError };
  accumulatedText = '';
  pendingCarry = '';
  matchPos = 0;
