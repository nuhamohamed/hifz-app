...
export async function getTodayPortion(db, userId, dueQuizMinutes = 0) {
  ...
  if (scheduled) {
    const { data: progress } = await supabase
      ...
      .eq('user_id', userId)
      .eq('juz_number', scheduled.juz_number)
      .maybeSingle();
...
export async function getTodayPlan(db, userId) {
  ...
  const portion = await getTodayPortion(db, userId, dueQuizMinutes);
  ...
  return {
    ...
    estimateMinutes: sessionEstimateMinutes(
      dueQuizMinutes,
      portion.recitationMinutes ?? 0,
      portion.pages ?? 0
    ),
    juzWaiting: await countJuzWaiting(userId, today, portion),
  };
}

/**
 * Juz with work due beyond the one being done today.
 *
 * Reported as a plain fact rather than a warning, and nothing is ever dropped
 * to make it smaller: a backlog is worked through oldest first and shrinks by
 * being done.
 */
async function countJuzWaiting(userId, today, portion) {
  ...
  for (const row of scheduled ?? []) waiting.add(row.juz_number);
  ...
  for (const row of due ?? []) waiting.add(row.juz_number);

  if (portion?.juzNumber != null) waiting.delete(portion.juzNumber);
  return waiting.size;
}
