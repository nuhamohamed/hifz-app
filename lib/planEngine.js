...
export async function getTodayPortions(db, userId, dueQuizMinutes = 0) {
  ...
  if (scheduled?.length) {
    // A juz already recited today is finished for the day, however many of its
    // portions are still pending. Without this the next one appears the moment
    // a session ends and someone behind could be walked through their whole
    // backlog in an afternoon, which is the cramming the plan is meant to
    // prevent. A second *juz* falling on the same day is different: that is
    // genuinely today's work and is listed.
    //
    // Filtered to real recitation. A quiz-only session carries juz 1 in its
    // portion columns as filler, and counting it would silently retire juz 1
    // for the day.
