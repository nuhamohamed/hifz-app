 ...
 * >}
 */
export async function getTodayPortions(db, userId, dueQuizMinutes = 0, options = {}) {
  // Every date comparison below runs against this rather than "now", so the
  // same function can answer "what does tomorrow look like" for the session
  // summary. It is the whole point of the parameter: the summary used to read
  // scheduled_portions directly and could name a portion the agenda then re-cut
  // or, on a two-juz day, mention one of them and not the other.
  //
  // coveredBySession is the session that has just finished but has not been
  // scored yet, because scoring waits for the person's corrections. Its rows
  // are still pending, so a preview of tomorrow would otherwise offer back the
  // portion they have this minute recited. Filtered with the same predicate
  // scoring uses to mark them done.
  const { onDate, coveredBySession = null } = options;
  const today = onDate ?? getTodayDateString();
  ...
  const { data: doneToday, error: doneError } = await supabase
    ...
    .select('juz_number')
    .eq('user_id', userId)
    // Nothing has been recited on a day that has not arrived, so for a preview
    // this is correctly empty and every juz stays eligible.
    .eq('date', today)
    .eq('status', 'complete')
    .eq('type', 'revision');
  ...
  if (scheduled?.length) {
    ...
    for (const row of scheduled) {
      if (
        coveredBySession &&
        row.juz_number === coveredBySession.juzNumber &&
        row.portion_start_ayah >= coveredBySession.portionStartAyah &&
        row.portion_start_ayah <= coveredBySession.portionEndAyah
      ) {
        continue;
      }
...
async function fetchMistakeCountForSession(sessionId) {
  const { count, error } = await supabase
    ...
    // Cleared misflags do not count against the session. This number decides
    // whether the portion halves and how far out the juz is pushed, so counting
    // words the app misheard would penalise someone for its own error.
...
async function getOrCreateJuzProgress(userId, juzNumber) {
  ...
  if (insertError) {
    // Someone else created it between the select above and this insert. There
    // is a unique index on (user_id, juz_number), so the race ends in a
    // constraint violation rather than a duplicate row, and the right answer is
    // simply the row they wrote. Re-reading is safe: the losing caller wanted a
    // row for this juz and there now is one, with the same defaults it would
    // have written itself.
    //
    ...
    if (insertError.code === '23505') {
      ...
      if (!reselectError && raced) return raced;
    }
...
/**
 * What a given day's agenda will actually hold.
 *
 * The session summary used to answer this with its own query straight against
 * scheduled_portions, and the two disagreed in three ways. It took `.limit(1)`,
 * so a day carrying two juz was announced as one. It read the stored range,
 * while the agenda re-sizes every row to fit the day. And it costed nothing for
 * the review, while the agenda pays for the quiz first and gives the portion
 * what is left.
 *
 * Now both screens ask this. Two screens quietly disagreeing about the same day
 * is worse than either being wrong on its own, because nothing looks broken.
 *
 * `justFinished` is the session that has this minute completed. It has not been
 * scored yet, since scoring waits for the person's corrections on the summary,
 * so its scheduled rows are still pending and would otherwise be offered back
 * to them tomorrow.
 */
export async function getPortionsForDate(db, userId, date, justFinished = null) {
  const { data: user } = await supabase
    .from('users')
    .select('session_minutes, avg_minutes_per_page')
    .eq('id', userId)
  ...
  const { minutes: dueQuizMinutes } = await dueQuizCost(
    db,
    userId,
    minutesPerPage,
    date
  ...
  const { portions } = await getTodayPortions(db, userId, dueQuizMinutes, {
    onDate: date,
    coveredBySession: justFinished,
  });
...
/**
 * Score every finished session that has not been scored yet.
 *
 * The summary screen used to do this on mount, which put it before the one
 * thing the summary exists for. Someone whose recitation was misheard five
 * times had the session judged as five mistakes, the portion halved and the
 * juz pulled forward, and only then were they shown the list and invited to
 * say the app had misheard them. Clearing all five removed the mistakes and
 * left every consequence of them in place.
 *
 * Scoring happens here instead, on the way back to the Today screen, by which
 * point the corrections are already in the table. plan_applied is still the
 * guard, so a session is scored exactly once whichever route gets here first.
 *
 * Quiz-only days are skipped: they have no portion, and their juz and ayah
 * columns are filler.
 */
export async function applyPendingSessionPlans(db, userId) {
  const { data: pending, error } = await supabase
    ...
    // Oldest first: a backlog of unscored sessions has to be applied in the
    // order it happened, because each one's interval builds on the last.
  ...
  for (const session of pending) {
    // Claimed before the work, so two screens loading at once cannot both
    // score the same session. A row that fails the filter was already taken.
    ...
    try {
      await updateJuzProgressAfterSession(
        ...
        session.portion_end_ayah,
        getJuzTotalAyahs(session.juz_number)
      );
    } catch (err) {
      // Hand the session back rather than leaving it marked as scored when it
      // was not, or its portions would never come round again.
      await supabase
        .from('sessions')
        .update({ plan_applied: false })
        .eq('id', session.id);
      console.error('[plan] could not score a session:', err?.message);
    }
  }
}

/**
 * What the interval WILL be, without writing anything.
 *
 * The summary tells someone when a finished juz comes back, and it used to read
 * that off juz_progress because scoring had already happened by the time it
 * rendered. Now that scoring waits for their corrections, the stored value is
 * still the previous pass's, so the number has to be computed rather than read.
 * Same inputs and same function as the real thing, so the two cannot disagree.
 */
export async function previewNextInterval(db, userId, sessionId, juzNumber) {
  ...
  const { data: progress } = await supabase
    .from('juz_progress')
    .select('pass_mistakes, interval_days')
    .eq('user_id', userId)
    .eq('juz_number', juzNumber)
    .maybeSingle();
  ...
  const ceiling = intervalCeilingDays(
    ...
    settings?.session_minutes ?? DEFAULT_SESSION_MINUTES,
    settings?.avg_minutes_per_page ?? DEFAULT_MINUTES_PER_PAGE
  );

  return getNextReviewDate(passMistakes, progress?.interval_days ?? null, ceiling);
}
...
/**
 * Pages in the range a session already under way is committed to.
 *
 * Read off the session's own row rather than off the plan, because the whole
 * reason this exists is that the plan can report nothing for a day that plainly
 * holds work. See the note in getTodayPlan().
 *
 * Zero when there is no such session, and zero rather than a throw when the
 * page map cannot be read: a missing estimate costs a detail line, and the
 * callers already treat zero as "say nothing about the time".
 */
async function activeSessionPages(db, userId, date) {
  const { data, error } = await supabase
    .from('sessions')
    .select('juz_number, portion_start_ayah, portion_end_ayah')
    .eq('user_id', userId)
    .eq('date', date)
    .in('status', ['paused', 'in_progress'])
    // A quiz-only session carries juz 1 and portion 1-1 as filler, and costing
    // that filler as if it were recitation would invent a portion nobody has.
    .neq('type', 'quiz_only')
    .limit(1)
    .maybeSingle();

  if (error || !data?.portion_start_ayah || !data?.portion_end_ayah) return 0;

  try {
    return await portionPagesFor(
      db,
      data.juz_number ?? 1,
      data.portion_start_ayah,
      data.portion_end_ayah
    );
  } catch {
    return 0;
  }
...
export async function getTodayPlan(db, userId) {
  ...
  let estimateMinutes = sessionEstimateMinutes(
    dueQuizMinutes,
  ...
  // and it has to be costed even when the schedule is looking the other way.
  //
  // Starting a session brings its scheduled row forward without consuming it:
  // the row stays pending on its original future date until the session
  // completes. getTodayPortions() then finds nothing for today, and correctly
  // so on its own terms. Its scheduled branch filters to scheduled_date <=
  // today, which a row booked for tomorrow fails, and its improvised branch
  // stands down for any juz that already holds a pending row, which is right
  // for a juz deliberately set to begin later.
  //
  // The result was an empty portion list for a day with a half-finished
  // portion on it, and an estimate of zero minutes to go with it. Zero is not a
  // harmless number here: every line on the Today screen that mentions a time
  // is guarded on the estimate being truthy, so the banner went silent about
  // both the pages and the minutes for someone paused six ayahs into a
  // 148-ayah portion.
  //
  // Costed from the session's own committed range, which is the one thing on a
  // paused day that is not in doubt. Only when the plan found nothing: a day
  // that did produce portions has already counted this work.
  if (portions.length === 0) {
    const activePages = await activeSessionPages(db, userId, today);
    if (activePages > 0) {
      estimateMinutes = sessionEstimateMinutes(
        dueQuizMinutes,
        activePages * minutesPerPage,
        activePages
      );
    }
  }
