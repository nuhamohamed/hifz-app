 ...
 * Rounded **up** to the nearest quarter hour, matching the slider: too much
 * time is a day that ends early, too little is a backlog that never clears.
 *
 * 1 to 8 juz gets 15 minutes, 15 juz gets 30, all 30 juz gets 60.
 */
export function recommendedSessionMinutes(juzCount, minutesPerPage) {
  ...
  // Quarter hours, matching the slider. Rounding to 5 put the suggestion on
  // numbers the slider could no longer reach, so it snapped away the moment
  // anyone touched it.
  return Math.max(MIN_SESSION_MINUTES, Math.ceil(total / 15) * 15);
}

/**
 * The suggestion as it is worded on screen.
 *
 * Lives here rather than in the two screens that show it, because the same
 * sentence in two files is how two sentences start disagreeing.
 *
 * "At least" on purpose. The wording used to swap between three variants
 * depending on where the slider sat, one of which promised that the suggested
 * number "covers everything you have memorised on a monthly round". That is
 * true at about one mistake per page and false above it, and the person reading
 * it has no way to tell which case they are in. A floor is a thing the app can
 * actually stand behind.
 */
export function suggestionLine(recommended, juzCount) {
  const memorized = juzCount === 1 ? 'the juz' : `the ${juzCount} juz`;
  return `We suggest at least ${recommended} minutes for ${memorized} you have memorized.`;
}
