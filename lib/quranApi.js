...
/**
 * Fetch Uthmanic Arabic text for a single ayah from quran.com.
 *
 * @param {number} surahNumber
 * @param {number} ayahNumber
 * @returns {Promise<{ text: string, words: { text: string }[] }>}
 */
export async function getAyah(surahNumber, ayahNumber) {
  const ayahKey = `${surahNumber}:${ayahNumber}`;
  const url = `${QURAN_API_BASE}/verses/by_key/${ayahKey}?words=true&word_fields=text_uthmani`;
  ...
  const words = rawWords
    ...
    .map((word) => ({ text: word.text_uthmani }));
