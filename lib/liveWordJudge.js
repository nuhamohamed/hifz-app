 ...
 * @param {function(number[]): void} [opts.onMarksChange] current wrong/missing
 *   expected-word indices in the stable region (recomputed per update, may shrink
 *   if the transcriber revises a word to correct)
 */
export function createLiveJudge({ getExpectedWords, onMistake, onMarksChange }) {
  ...
  return {
    update(liveText) {
      ...
      onMarksChange?.(wrongEntries.map((s) => s.index));
