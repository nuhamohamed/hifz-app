...
/**
 * Each ayah's share of its page, for an arbitrary set of ayahs.
 *
 * `getJuzAyahPages` answers the same question for a whole juz, which is what
 * portion sizing needs. The quiz needs it for a scattered handful instead: due
 * items come from wherever mistakes were made, so they cross juz freely and
 * there is no range to sweep.
 *
 * This is what makes the quiz's cost real rather than assumed. A flat per-item
 * estimate is wrong by a factor of 40 at the extremes, since An-Naba 1 is 3
 * seconds and Al-Baqarah 282 is two minutes.
 *
 * Chunked because each pair costs two bound parameters and SQLite refuses more
 * than 999 in one statement. A quiz is 10 to 15 items, so this only matters for
 * someone returning to a long backlog.
 *
 * @param {import('expo-sqlite').SQLiteDatabase} db
 * @param {{ surah: number, ayah: number }[]} refs
 * @returns {Promise<Map<string, number>>} keyed `"surah:ayah"`
 */
export async function getAyahPageShares(db, refs) {
  const shares = new Map();
  if (!refs?.length) return shares;

  // Deduplicated first: the same ayah can be due from two different mistakes.
  const unique = new Map();
  for (const r of refs) unique.set(`${r.surah}:${r.ayah}`, r);
  ...
  for (let i = 0; i < list.length; i += CHUNK) {
    ...
    const where = batch.map(() => '(surah = ? AND ayah = ?)').join(' OR ');
    const params = batch.flatMap((r) => [r.surah, r.ayah]);

    // Uses idx_words_loc, the same index getJuzAyahPages relies on.
    const rows = await db.getAllAsync(
      `SELECT surah, ayah, COUNT(*) AS wordCount, MIN(id) AS firstWordId
         FROM words
        WHERE ${where}
    ...
    for (const r of rows) {
      const page = pageForWordId(lines, r.firstWordId);
      const pageWords = totals.get(page) ?? r.wordCount;
      shares.set(`${r.surah}:${r.ayah}`, r.wordCount / pageWords);
    }
  }
