...
/**
 * Short human-readable range label for a juz, e.g. "Al-Baqarah 142–252"
 * or "Al-Baqarah 253 – Ali Imran 92".
 */
export function getJuzRangeLabel(juzNumber) {
...
/**
 * Surah/ayah bounds for an entire juz (memorized portion).
 *
 * @param {number} juzNumber 1–30
 * @returns {{ surah_start: number, ayah_start: number, surah_end: number, ayah_end: number }}
 */
export function getJuzSurahRange(juzNumber) {
  const juz = JUZ_DATA[juzNumber - 1];
  if (!juz?.surahs?.length) {
    throw new Error(`Invalid juz number: ${juzNumber}`);
  }
  const first = juz.surahs[0];
  const last = juz.surahs[juz.surahs.length - 1];
  return {
    surah_start: first.surahNumber,
    ayah_start: first.startAyah,
    surah_end: last.surahNumber,
    ayah_end: last.endAyah,
  };
}
