...
export async function resetAccountData(userId) {
  ...
  const { error: userError } = await supabase
    ...
    .update({
      name: null,
      // Age went with everything else it was stored alongside, and did not.
      // The screen is called "Erase all my data" and the confirmation says
      // nothing can be recovered, so leaving a field behind made both untrue.
      // It is also the only way someone has to erase themselves, and the
      // privacy policy promises deletion of everything held about them.
      //
      // Keeping it was never protection against an under-13 re-entering a
      // different age: deleting the app and reinstalling gives a fresh
      // anonymous account anyway. The column allows null; the CHECK only
      // constrains a value when there is one.
      age: null,
